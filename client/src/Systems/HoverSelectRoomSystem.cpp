...
#include "HoverSelectRoomSystem.hpp"

namespace RT::Client::Systems {

    void HoverSelectRoomSystem::init(std::shared_ptr<GE::ECS::EntityManager> entityManager)
    {

    }

    void HoverSelectRoomSystem::update(std::shared_ptr<GE::ECS::EntityManager> entityManager)
    {
        std::string name = "";
        GE::ECS::ComponentManager &componentManager = entityManager->getComponentManager();
        std::unordered_map<RT::GE::ECS::Entity, std::shared_ptr<GE::ECS::Components::AComponent>, RT::GE::ECS::Entity::HashFunction> selectRooms = componentManager.getComponents<Components::SelectRoom>();
        for (auto &it : selectRooms) {
            std::shared_ptr<Components::SelectRoom> cButton = std::dynamic_pointer_cast<Components::SelectRoom>(it.second);
            ...
            if (cDrawable->getType() == RT::GE::ECS::Components::ADrawable::Spritesheet) {
                ...
                if (name == "assets/images/card_1.png" || name ==  "assets/images/card_2.png") {
                    ...
                    if (cButton->isSelect() == true)
                        cSpriteSheet->setIndex(RT::GE::Utils::Vector2<int>{0, 1});
                    else if (cButton->getHover(pos, dim) == true && cSpriteSheet->getIndex() != RT::GE::Utils::Vector2<int>{0, 2}) {
                        cSpriteSheet->setIndex(RT::GE::Utils::Vector2<int>{0, 1});
                    } else if (cButton->getHover(pos, dim) == false) {
                        cSpriteSheet->setIndex(RT::GE::Utils::Vector2<int>{0, 0});
                    }
                    if (_IdInstance->getId() != cButton->getId()) {
                        cSpriteSheet->setIndex(RT::GE::Utils::Vector2<int>{0, 0});
